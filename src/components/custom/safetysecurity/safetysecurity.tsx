...
// If Magic UI and Aceternity UI libraries are installed, import their components here
// Example (adjust imports based on your setup):
// import { ShimmerButton } from "magic-ui";
// import { Spotlight } from "aceternity-ui";
...
function SpotlightBG() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      <div className="absolute left-1/2 top-[-20%] h-[60vh] w-[60vh] -translate-x-1/2 rounded-full bg-gradient-to-br from-emerald-400/20 to-cyan-400/10 blur-3xl" />
      ...
      <div className="absolute left-[5%] top-[30%] h-40 w-40 rounded-full bg-emerald-500/10 blur-2xl" />
    </div>
  );
}
...
export default function SafetySecuritySection({ variant = "default" }: { variant?: "pg" | "hostel" | "apartment" | "default" }) {
  ...
  return (
    <section className="relative mx-auto max-w-7xl scroll-mt-24 rounded-3xl border border-emerald-500/10 bg-gradient-to-b from-white to-emerald-50/40 p-6 shadow-sm md:p-10">
      <SpotlightBG />

      {/* Header */}
      <div className="mb-8 flex flex-col items-start gap-3 md:mb-12 md:flex-row md:items-center md:justify-between">
        <div>
          <Badge variant="secondary" className="mb-3 rounded-full bg-emerald-100 text-emerald-700">Safety & Security</Badge>
      ...
      <div className="mb-10 grid grid-cols-2 gap-3 rounded-2xl border bg-white/60 p-4 backdrop-blur md:grid-cols-4">
        {[{ label: "Verified Homes", value: "12k+" }, { label: "Host Checks", value: "8k+" }, { label: "Cities", value: "140+" }, { label: "Avg. Rating", value: "4.7/5" }].map((item) => (
          <div key={item.label} className="text-center">
            <div className="text-xl font-bold md:text-2xl">{item.value}</div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {features.map((f, i) => (
          ...
          >
            <Card className="group h-full rounded-2xl border-emerald-500/10 bg-white/70 shadow-sm backdrop-blur transition hover:shadow-md">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card className="rounded-2xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <FileCheck className="h-5 w-5 text-emerald-600" /> Our safety checklist
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="overflow-hidden rounded-2xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <MapPin className="h-5 w-5 text-emerald-600" /> Neighborhood insights
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {/* CTA */}
      <div className="mt-8 flex flex-col items-center justify-between gap-4 rounded-2xl border bg-emerald-600/95 p-6 text-white md:flex-row">
        <div>
        ...
        <Shimmer>
          <Button asChild variant="secondary" className="rounded-2xl bg-white text-emerald-700 hover:bg-emerald-50">
